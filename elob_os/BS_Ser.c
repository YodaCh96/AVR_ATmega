...
DESCRIPTION:
    ...
    power of 2.
    Adapted to the ELOB Board by pst
...
// PIUS
static volatile unsigned char uart0_EventChar;
static volatile unsigned char uart1_EventChar;
...
static void PushError(const uint8_t UartErr, const uint8_t UartNo)
{
   // check for Frame or Overrun error
   if ( UartErr & (UART_FRAME_ERROR>>8) )
      error_put(FERR_UART0_FRAME+UartNo ); // Framing Error detected, i.e no stop bit detected 

   // Overrun, a character already present in the UART UDR register was 
   // not read by the interrupt handler before the next character arrived,
   // one or more received characters have been dropped
   if ( UartErr & (UART_OVERRUN_ERROR>>8)) 
      error_put(FERR_UART0_OVER+UartNo ); 

   // We are not reading the receive buffer fast enough,
   // one or more received character have been dropped 
   if ( UartErr & (UART_BUFFER_OVERFLOW>>8)) 
      error_put(FERR_UART0_BUFF+UartNo); 
}
...
ISR (USART0_RX_vect) // UART0_RECEIVE_INTERRUPT)	
{
    ...
    if(data == uart0_EventChar) {  // is it the char to release a event ?
       putQueue(EVENT_RXUART0);
    }


    if(lastRxError) {
	   PushError(lastRxError, 0);
    }
...
void uart_init(unsigned int baudrate)
{
   ...
   UART_RxTail = 0;
    ...
    #endif    
       UART0_UBRRL = (unsigned char) (baudrate&0x00FF);      // <- 0009 fuer 115k
...
uint8_t uart0_getToBuff(char* pBuff)
{
   uint16_t Val;
   uint8_t  Wr  = 0;

   while(!((Val = uart_getc()) & UART_NO_DATA)) { // data
      *(pBuff+Wr++) = (uint8_t)Val;
      *(pBuff+Wr) = 0;
   }
	return Wr;
}
...
void uart_putc( char data)
{
    ...
    while ( tmphead == UART_TxTail ) {
       PORTC ^= (1<<PC6);  // PIUS der kommt und dann haengt er
...
uint8_t uart1_getToBuff(char* pBuff)
{
   uint16_t Val;
   uint8_t  Wr  = 0;

   while(!((Val = uart1_getc()) & UART_NO_DATA)) { // data
      *(pBuff+Wr++) = (uint8_t)Val;
      *(pBuff+Wr) = 0;
   }
   return Wr;
...
ISR(UART1_RECEIVE_INTERRUPT)
{
   ...
   if ( tmphead == UART1_RxTail ) {
      ...
      //uart_putc(data);  // this is the best place to implement the echo 
        
      if(data == 0x13 || data == 0x11 || data == 0x1A) { // ctrl S oder Q
        // putQueue(EVENT_RXUART1);      // then send the event

      }

      if (data == uart1_EventChar) {    // is it the char to need event ?
         putQueue(EVENT_RXUART1);
      }
      if(lastRxError)
         PushError(lastRxError, 0);
   }
